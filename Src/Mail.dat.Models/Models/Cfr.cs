...
namespace Mail.dat
{
	...
	[PrimaryKey("Id")]
	public partial class Cfr : MaildatEntity
	{
		/// <summary>
		/// Job ID (CFR-1001)
		/// </summary>
		...
		[Column("ReserveCFR1109", Order = 14)]
		[MaxLength(20)]
		[Comment("CFR-1109")]
		...
		[Column("CFRRecordStatus", Order = 15)]
		[Required]
		[MaxLength(1)]
		...
		protected override ILoadError[] OnLoadData(int fileLineNumber, byte[] line)
		{
			...
			this.JobID = line.Parse<Cfr, string>(p => p.JobID, returnValue);
			this.CertificateOfMailingHeaderID = line.Parse<Cfr, string>(p => p.CertificateOfMailingHeaderID, returnValue);
			this.PieceID = line.Parse<Cfr, string>(p => p.PieceID, returnValue);
			this.ServiceType = line.Parse<Cfr, string>(p => p.ServiceType, returnValue);
			this.ServiceAdditionalType = line.Parse<Cfr, string>(p => p.ServiceAdditionalType, returnValue);
			this.ServiceStatedValue = line.Parse<Cfr, decimal>(p => p.ServiceStatedValue, returnValue);
			this.ServiceFee = line.Parse<Cfr, decimal>(p => p.ServiceFee, returnValue);
			this.SpecialFeesChargesServicesID = line.Parse<Cfr, string>(p => p.SpecialFeesChargesServicesID, returnValue);
			this.AmountDue = line.Parse<Cfr, decimal>(p => p.AmountDue, returnValue);
			this.FlexOptionA = line.Parse<Cfr, string>(p => p.FlexOptionA, returnValue);
			this.FlexOptionB = line.Parse<Cfr, string>(p => p.FlexOptionB, returnValue);
			this.FlexOptionC = line.Parse<Cfr, string>(p => p.FlexOptionC, returnValue);
			this.ReserveCFR1109 = line.Parse<Cfr, string>(p => p.ReserveCFR1109, returnValue);
			this.CFRRecordStatus = line.Parse<Cfr, string>(p => p.CFRRecordStatus, returnValue);
			this.ClosingCharacter = line.Parse<Cfr, string>(p => p.ClosingCharacter, returnValue);
