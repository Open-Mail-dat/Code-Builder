...
namespace Mail.dat
{
	...
	[PrimaryKey("Id")]
	public partial class Pbc : MaildatEntity
	{
		/// <summary>
		/// Job ID (PBC-1001)
		/// </summary>
		...
		[Column("WastedOrShortagePieceIndicator", Order = 7)]
		[MaxLength(1)]
		...
		[Column("IMpbBarcodeConstructCode", Order = 8)]
		[MaxLength(1)]
		...
		[Column("MIDInIMbIsMoveUpdateSupplier", Order = 9)]
		[MaxLength(1)]
		[AllowedValues("Y")]
		[Comment("PBC-1119")]
		...
		[Column("PBCRecordStatus", Order = 10)]
		[Required]
		[MaxLength(1)]
		...
		protected override ILoadError[] OnLoadData(int fileLineNumber, byte[] line)
		{
			...
			this.JobID = line.Parse<Pbc, string>(p => p.JobID, returnValue);
			this.PBCUniqueID = line.Parse<Pbc, int>(p => p.PBCUniqueID, returnValue);
			this.CQTDatabaseID = line.Parse<Pbc, int>(p => p.CQTDatabaseID, returnValue);
			this.PackageID = line.Parse<Pbc, string>(p => p.PackageID, returnValue);
			this.Barcode = line.Parse<Pbc, string>(p => p.Barcode, returnValue);
			this.WastedOrShortagePieceIndicator = line.Parse<Pbc, string>(p => p.WastedOrShortagePieceIndicator, returnValue);
			this.IMpbBarcodeConstructCode = line.Parse<Pbc, string>(p => p.IMpbBarcodeConstructCode, returnValue);
			this.MIDInIMbIsMoveUpdateSupplier = line.Parse<Pbc, string>(p => p.MIDInIMbIsMoveUpdateSupplier, returnValue);
			this.PBCRecordStatus = line.Parse<Pbc, string>(p => p.PBCRecordStatus, returnValue);
			this.ClosingCharacter = line.Parse<Pbc, string>(p => p.ClosingCharacter, returnValue);
